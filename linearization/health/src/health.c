...
struct Village *alloc_tree(int level, int label, struct Village *back) {
  if (level == 0)
    ...
    struct Village       *fval[4];
      ...
      new = (struct Village *)alloc(sizeof(struct Village));
...
struct Patient *generate_patient(struct Village *village) 
{
  ...
  if (rand > 0.666) {
   // patient = (struct Patient *)malloc(sizeof(struct Patient));
     patient = (struct Patient *)alloc(sizeof(struct Patient));
...
{ 
  ...
  initBumpAlloc(pow(5, max_level)* sizeof(struct Village)+(sizeof(struct Patient)+sizeof(struct List))* 5000000);
