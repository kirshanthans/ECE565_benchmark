...
{ \
    h = (HANDLE *) alloc(sizeof(struct node)); \
      h->value = v; \
	h->left = NIL; \
	  h->right = NIL; \
	  };
...
/* Generate the nth randomX # */
int skiprand(int seed, int n) {
  for (; n; n--) seed=randomX(seed);
  return seed;
}

int randomX(int seed) {
...
int main(int argc, char **argv) {
  ...
  printf("Bisort with %d size of dim %d\n", n, NDim);
    ...
    initBumpAlloc(2*n*sizeof(struct node));
  h = RandTree(n,12345768,0,0);
  sval = randomX(245867) % RANGE;
  if (flag) {
    InOrder(h);
    printf("%d\n",sval);
  }
